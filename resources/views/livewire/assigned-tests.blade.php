<div>
        ...
        <div class="table-responsive">
            <table class="table mb-0">
                ...
                <tbody>
                        ...
                        <tr class="border-top border-light">
                            ...
                            <td class="py-3">
                                @php
                                    ...
                                    $badgeText = ucfirst($item->estado);
                                    
                                    if ($item->estado == 'realizado') {
                                        $badgeClass = 'badge-success';
                                    } elseif ($item->estado == 'en_progreso') {
                                        $badgeClass = 'badge-info';
                                    }
                                @endphp
                                <span class="badge badge-pill {{ $badgeClass }} px-3 py-2">{{ $badgeText }}</span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-4 text-muted">No hay tests asignados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    {{-- Modal Detalle Test permanece sin cambios --}}
                ...
                <div class="modal-body">
                    @if($asignacionSeleccionada)
                        <div class="bg-light p-4 rounded">
                            ...
                            <p><strong>Enlace:</strong> <a href="{{ url('/test/ingresar') }}" target="_blank">{{ url('/test/ingresar') }}</a></p>
                        </div>
                    @else
                        <div class="alert alert-warning m-0">No se han encontrado detalles del test seleccionado.</div>
                    @endif
                </div>
